...

{
  ...
  home.activation.createPiMemoryDir = config.lib.dag.entryAfter [ "writeBoundary" ] ''
    $DRY_RUN_CMD mkdir -p "$HOME/pi-memory"
  '';

  # Remove old non-symlink files before creating symlinks
  home.activation.cleanupPiFiles = config.lib.dag.entryBefore [ "linkGeneration" ] ''
  ...
  home.file.".pi/agent/settings.json".source =
    config.lib.file.mkOutOfStoreSymlink "${config.resources.paths.piConfig}/settings/${configName}/settings.json";

  # Symlink skills directory for instant changes without rebuild
  home.file.".pi/agent/skills".source =
    config.lib.file.mkOutOfStoreSymlink "${config.resources.paths.piConfig}/skills";

  # Symlink extensions directory for instant changes without rebuild
  home.file.".pi/agent/extensions".source =
    config.lib.file.mkOutOfStoreSymlink "${config.resources.paths.piConfig}/extensions";

  # Symlink models.json for instant changes without rebuild
  home.file.".pi/agent/models.json".source =
    config.lib.file.mkOutOfStoreSymlink "${config.resources.paths.piConfig}/models.json";
}
